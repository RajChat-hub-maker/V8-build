
'use strict';

let externrefs = {};
let externsym = Symbol("externref");
function externref(s) {
  if (! (s in externrefs)) externrefs[s] = {[externsym]: s};
  return externrefs[s];
}
function is_externref(x) {
  return (x !== null && externsym in x) ? 1 : 0;
}
function is_funcref(x) {
  return typeof x === "function" ? 1 : 0;
}
function eq_externref(x, y) {
  return x === y ? 1 : 0;
}
function eq_funcref(x, y) {
  return x === y ? 1 : 0;
}

let spectest = {
  externref: externref,
  is_externref: is_externref,
  is_funcref: is_funcref,
  eq_externref: eq_externref,
  eq_funcref: eq_funcref,
  print: console.log.bind(console),
  print_i32: console.log.bind(console),
  print_i64: console.log.bind(console),
  print_i32_f32: console.log.bind(console),
  print_f64_f64: console.log.bind(console),
  print_f32: console.log.bind(console),
  print_f64: console.log.bind(console),
  global_i32: 666,
  global_i64: 666n,
  global_f32: 666.6,
  global_f64: 666.6,
  table: new WebAssembly.Table({initial: 10, maximum: 20, element: 'anyfunc'}),
  memory: new WebAssembly.Memory({initial: 1, maximum: 2})
};

let handler = {
  get(target, prop) {
    return (prop in target) ?  target[prop] : {};
  }
};
let registry = new Proxy({spectest}, handler);

function register(name, instance) {
  registry[name] = instance.exports;
}

function module(bytes, valid = true) {
  let buffer = new ArrayBuffer(bytes.length);
  let view = new Uint8Array(buffer);
  for (let i = 0; i < bytes.length; ++i) {
    view[i] = bytes.charCodeAt(i);
  }
  let validated;
  try {
    validated = WebAssembly.validate(buffer);
  } catch (e) {
    throw new Error("Wasm validate throws");
  }
  if (validated !== valid) {
    throw new Error("Wasm validate failure" + (valid ? "" : " expected"));
  }
  return new WebAssembly.Module(buffer);
}

function instance(bytes, imports = registry) {
  return new WebAssembly.Instance(module(bytes), imports);
}

function call(instance, name, args) {
  return instance.exports[name](...args);
}

function get(instance, name) {
  let v = instance.exports[name];
  return (v instanceof WebAssembly.Global) ? v.value : v;
}

function exports(instance) {
  return {module: instance.exports, spectest: spectest};
}

function run(action) {
  action();
}

function assert_malformed(bytes) {
  try { module(bytes, false) } catch (e) {
    if (e instanceof WebAssembly.CompileError) return;
  }
  throw new Error("Wasm decoding failure expected");
}

function assert_invalid(bytes) {
  try { module(bytes, false) } catch (e) {
    if (e instanceof WebAssembly.CompileError) return;
  }
  throw new Error("Wasm validation failure expected");
}

function assert_unlinkable(bytes) {
  let mod = module(bytes);
  try { new WebAssembly.Instance(mod, registry) } catch (e) {
    if (e instanceof WebAssembly.LinkError) return;
  }
  throw new Error("Wasm linking failure expected");
}

function assert_uninstantiable(bytes) {
  let mod = module(bytes);
  try { new WebAssembly.Instance(mod, registry) } catch (e) {
    if (e instanceof WebAssembly.RuntimeError) return;
  }
  throw new Error("Wasm trap expected");
}

function assert_trap(action) {
  try { action() } catch (e) {
    if (e instanceof WebAssembly.RuntimeError) return;
  }
  throw new Error("Wasm trap expected");
}

let StackOverflow;
try { (function f() { 1 + f() })() } catch (e) { StackOverflow = e.constructor }

function assert_exhaustion(action) {
  try { action() } catch (e) {
    if (e instanceof StackOverflow) return;
  }
  throw new Error("Wasm resource exhaustion expected");
}

function assert_return(action, ...expected) {
  let actual = action();
  if (actual === undefined) {
    actual = [];
  } else if (!Array.isArray(actual)) {
    actual = [actual];
  }
  if (actual.length !== expected.length) {
    throw new Error(expected.length + " value(s) expected, got " + actual.length);
  }
  for (let i = 0; i < actual.length; ++i) {
    switch (expected[i]) {
      case "nan:canonical":
      case "nan:arithmetic":
      case "nan:any":
        // Note that JS can't reliably distinguish different NaN values,
        // so there's no good way to test that it's a canonical NaN.
        if (!Number.isNaN(actual[i])) {
          throw new Error("Wasm return value NaN expected, got " + actual[i]);
        };
        return;
      case "ref.func":
        if (typeof actual[i] !== "function") {
          throw new Error("Wasm function return value expected, got " + actual[i]);
        };
        return;
      case "ref.extern":
        if (actual[i] === null) {
          throw new Error("Wasm reference return value expected, got " + actual[i]);
        };
        return;
      default:
        if (!Object.is(actual[i], expected[i])) {
          throw new Error("Wasm return value " + expected[i] + " expected, got " + actual[i]);
        };
    }
  }
}

// memory_grow.wast:1
let $1 = instance("");

// memory_grow.wast:6
assert_return(() => call($1, "grow", [0]), 0);

// memory_grow.wast:7
assert_return(() => call($1, "grow", [1]), 0);

// memory_grow.wast:8
assert_return(() => call($1, "grow", [0]), 1);

// memory_grow.wast:9
assert_return(() => call($1, "grow", [2]), 1);

// memory_grow.wast:10
assert_return(() => call($1, "grow", [800]), 3);

// memory_grow.wast:11
assert_return(() => call($1, "grow", [65_536]), -1);

// memory_grow.wast:12
assert_return(() => call($1, "grow", [64_736]), -1);

// memory_grow.wast:13
assert_return(() => call($1, "grow", [1]), 803);

// memory_grow.wast:15
let $2 = instance("");

// memory_grow.wast:20
assert_return(() => call($2, "grow", [0]), 0);

// memory_grow.wast:21
assert_return(() => call($2, "grow", [1]), 0);

// memory_grow.wast:22
assert_return(() => call($2, "grow", [1]), 1);

// memory_grow.wast:23
assert_return(() => call($2, "grow", [2]), 2);

// memory_grow.wast:24
assert_return(() => call($2, "grow", [6]), 4);

// memory_grow.wast:25
assert_return(() => call($2, "grow", [0]), 10);

// memory_grow.wast:26
assert_return(() => call($2, "grow", [1]), -1);

// memory_grow.wast:27
assert_return(() => call($2, "grow", [65_536]), -1);

// memory_grow.wast:32
let $3 = instance("");

// memory_grow.wast:53
assert_return(() => call($3, "check-memory-zero", [0, 65_535]), 0);

// memory_grow.wast:54
assert_return(() => call($3, "grow", [1]), 1);

// memory_grow.wast:55
assert_return(() => call($3, "check-memory-zero", [65_536, 131_071]), 0);

// memory_grow.wast:56
assert_return(() => call($3, "grow", [1]), 2);

// memory_grow.wast:57
assert_return(() => call($3, "check-memory-zero", [131_072, 196_607]), 0);

// memory_grow.wast:58
assert_return(() => call($3, "grow", [1]), 3);

// memory_grow.wast:59
assert_return(() => call($3, "check-memory-zero", [196_608, 262_143]), 0);

// memory_grow.wast:60
assert_return(() => call($3, "grow", [1]), 4);

// memory_grow.wast:61
assert_return(() => call($3, "check-memory-zero", [262_144, 327_679]), 0);

// memory_grow.wast:62
assert_return(() => call($3, "grow", [1]), 5);

// memory_grow.wast:63
assert_return(() => call($3, "check-memory-zero", [327_680, 393_215]), 0);

// memory_grow.wast:68
let $4 = instance("");

// memory_grow.wast:85
assert_return(() => call($4, "size", []), 0);

// memory_grow.wast:86
assert_trap(() => call($4, "store_at_zero", []));

// memory_grow.wast:87
assert_trap(() => call($4, "load_at_zero", []));

// memory_grow.wast:88
assert_trap(() => call($4, "store_at_page_size", []));

// memory_grow.wast:89
assert_trap(() => call($4, "load_at_page_size", []));

// memory_grow.wast:90
assert_return(() => call($4, "grow", [1]), 0);

// memory_grow.wast:91
assert_return(() => call($4, "size", []), 1);

// memory_grow.wast:92
assert_return(() => call($4, "load_at_zero", []), 0);

// memory_grow.wast:93
assert_return(() => call($4, "store_at_zero", []));

// memory_grow.wast:94
assert_return(() => call($4, "load_at_zero", []), 2);

// memory_grow.wast:95
assert_trap(() => call($4, "store_at_page_size", []));

// memory_grow.wast:96
assert_trap(() => call($4, "load_at_page_size", []));

// memory_grow.wast:97
assert_return(() => call($4, "grow", [4]), 1);

// memory_grow.wast:98
assert_return(() => call($4, "size", []), 5);

// memory_grow.wast:99
assert_return(() => call($4, "load_at_zero", []), 2);

// memory_grow.wast:100
assert_return(() => call($4, "store_at_zero", []));

// memory_grow.wast:101
assert_return(() => call($4, "load_at_zero", []), 2);

// memory_grow.wast:102
assert_return(() => call($4, "load_at_page_size", []), 0);

// memory_grow.wast:103
assert_return(() => call($4, "store_at_page_size", []));

// memory_grow.wast:104
assert_return(() => call($4, "load_at_page_size", []), 3);

// memory_grow.wast:109
let $5 = instance("\x00\x61\x73\x6d\x01\x00\x00\x00\x05\x86\x80\x80\x80\x00\x02\x01\x02\x05\x00\x00\x07\x8f\x80\x80\x80\x00\x02\x04\x6d\x65\x6d\x31\x02\x00\x04\x6d\x65\x6d\x32\x02\x01");

// memory_grow.wast:113
register("M", $5)

// memory_grow.wast:115
let $6 = instance("");

// memory_grow.wast:140
assert_return(() => call($6, "size1", []), 2);

// memory_grow.wast:141
assert_return(() => call($6, "size2", []), 0);

// memory_grow.wast:142
assert_return(() => call($6, "size3", []), 3);

// memory_grow.wast:143
assert_return(() => call($6, "size4", []), 4);

// memory_grow.wast:145
assert_return(() => call($6, "grow1", [1]), 2);

// memory_grow.wast:146
assert_return(() => call($6, "size1", []), 3);

// memory_grow.wast:147
assert_return(() => call($6, "size2", []), 0);

// memory_grow.wast:148
assert_return(() => call($6, "size3", []), 3);

// memory_grow.wast:149
assert_return(() => call($6, "size4", []), 4);

// memory_grow.wast:151
assert_return(() => call($6, "grow1", [2]), 3);

// memory_grow.wast:152
assert_return(() => call($6, "size1", []), 5);

// memory_grow.wast:153
assert_return(() => call($6, "size2", []), 0);

// memory_grow.wast:154
assert_return(() => call($6, "size3", []), 3);

// memory_grow.wast:155
assert_return(() => call($6, "size4", []), 4);

// memory_grow.wast:157
assert_return(() => call($6, "grow1", [1]), -1);

// memory_grow.wast:158
assert_return(() => call($6, "size1", []), 5);

// memory_grow.wast:159
assert_return(() => call($6, "size2", []), 0);

// memory_grow.wast:160
assert_return(() => call($6, "size3", []), 3);

// memory_grow.wast:161
assert_return(() => call($6, "size4", []), 4);

// memory_grow.wast:163
assert_return(() => call($6, "grow2", [10]), 0);

// memory_grow.wast:164
assert_return(() => call($6, "size1", []), 5);

// memory_grow.wast:165
assert_return(() => call($6, "size2", []), 10);

// memory_grow.wast:166
assert_return(() => call($6, "size3", []), 3);

// memory_grow.wast:167
assert_return(() => call($6, "size4", []), 4);

// memory_grow.wast:169
assert_return(() => call($6, "grow3", [268_435_456]), -1);

// memory_grow.wast:170
assert_return(() => call($6, "size1", []), 5);

// memory_grow.wast:171
assert_return(() => call($6, "size2", []), 10);

// memory_grow.wast:172
assert_return(() => call($6, "size3", []), 3);

// memory_grow.wast:173
assert_return(() => call($6, "size4", []), 4);

// memory_grow.wast:175
assert_return(() => call($6, "grow3", [3]), 3);

// memory_grow.wast:176
assert_return(() => call($6, "size1", []), 5);

// memory_grow.wast:177
assert_return(() => call($6, "size2", []), 10);

// memory_grow.wast:178
assert_return(() => call($6, "size3", []), 6);

// memory_grow.wast:179
assert_return(() => call($6, "size4", []), 4);

// memory_grow.wast:181
assert_return(() => call($6, "grow4", [1]), 4);

// memory_grow.wast:182
assert_return(() => call($6, "grow4", [1]), -1);

// memory_grow.wast:183
assert_return(() => call($6, "size1", []), 5);

// memory_grow.wast:184
assert_return(() => call($6, "size2", []), 10);

// memory_grow.wast:185
assert_return(() => call($6, "size3", []), 6);

// memory_grow.wast:186
assert_return(() => call($6, "size4", []), 5);

// memory_grow.wast:191
let $7 = instance("");

// memory_grow.wast:349
assert_return(() => call($7, "as-br-value", []), 1);

// memory_grow.wast:351
assert_return(() => call($7, "as-br_if-cond", []));

// memory_grow.wast:352
assert_return(() => call($7, "as-br_if-value", []), 1);

// memory_grow.wast:353
assert_return(() => call($7, "as-br_if-value-cond", []), 6);

// memory_grow.wast:355
assert_return(() => call($7, "as-br_table-index", []));

// memory_grow.wast:356
assert_return(() => call($7, "as-br_table-value", []), 1);

// memory_grow.wast:357
assert_return(() => call($7, "as-br_table-value-index", []), 6);

// memory_grow.wast:359
assert_return(() => call($7, "as-return-value", []), 1);

// memory_grow.wast:361
assert_return(() => call($7, "as-if-cond", []), 0);

// memory_grow.wast:362
assert_return(() => call($7, "as-if-then", []), 1);

// memory_grow.wast:363
assert_return(() => call($7, "as-if-else", []), 1);

// memory_grow.wast:365
assert_return(() => call($7, "as-select-first", [0, 1]), 1);

// memory_grow.wast:366
assert_return(() => call($7, "as-select-second", [0, 0]), 1);

// memory_grow.wast:367
assert_return(() => call($7, "as-select-cond", []), 0);

// memory_grow.wast:369
assert_return(() => call($7, "as-call-first", []), -1);

// memory_grow.wast:370
assert_return(() => call($7, "as-call-mid", []), -1);

// memory_grow.wast:371
assert_return(() => call($7, "as-call-last", []), -1);

// memory_grow.wast:373
assert_return(() => call($7, "as-call_indirect-first", []), -1);

// memory_grow.wast:374
assert_return(() => call($7, "as-call_indirect-mid", []), -1);

// memory_grow.wast:375
assert_return(() => call($7, "as-call_indirect-last", []), -1);

// memory_grow.wast:376
assert_trap(() => call($7, "as-call_indirect-index", []));

// memory_grow.wast:378
assert_return(() => call($7, "as-local.set-value", []));

// memory_grow.wast:379
assert_return(() => call($7, "as-local.tee-value", []), 1);

// memory_grow.wast:380
assert_return(() => call($7, "as-global.set-value", []));

// memory_grow.wast:382
assert_return(() => call($7, "as-load-address", []), 0);

// memory_grow.wast:383
assert_return(() => call($7, "as-loadN-address", []), 0);

// memory_grow.wast:384
assert_return(() => call($7, "as-store-address", []));

// memory_grow.wast:385
assert_return(() => call($7, "as-store-value", []));

// memory_grow.wast:386
assert_return(() => call($7, "as-storeN-address", []));

// memory_grow.wast:387
assert_return(() => call($7, "as-storeN-value", []));

// memory_grow.wast:389
assert_return(() => call($7, "as-unary-operand", []), 31);

// memory_grow.wast:391
assert_return(() => call($7, "as-binary-left", []), 11);

// memory_grow.wast:392
assert_return(() => call($7, "as-binary-right", []), 9);

// memory_grow.wast:394
assert_return(() => call($7, "as-test-operand", []), 0);

// memory_grow.wast:396
assert_return(() => call($7, "as-compare-left", []), 1);

// memory_grow.wast:397
assert_return(() => call($7, "as-compare-right", []), 1);

// memory_grow.wast:399
assert_return(() => call($7, "as-memory.grow-size", []), 1);

// memory_grow.wast:401
let $8 = instance("");
let $Mgm = $8;

// memory_grow.wast:405
register("grown-memory", $Mgm)

// memory_grow.wast:406
assert_return(() => call($Mgm, "grow", []), 1);

// memory_grow.wast:407
let $9 = instance("");
let $Mgim1 = $9;

// memory_grow.wast:412
register("grown-imported-memory", $Mgim1)

// memory_grow.wast:413
assert_return(() => call($Mgim1, "grow", []), 2);

// memory_grow.wast:414
let $10 = instance("");
let $Mgim2 = $10;

// memory_grow.wast:419
assert_return(() => call($Mgim2, "size", []), 3);

// memory_grow.wast:424
let $11 = instance("");

// memory_grow.wast:439
assert_return(() => call($11, "size1", []), 1);

// memory_grow.wast:440
assert_return(() => call($11, "size2", []), 2);

// memory_grow.wast:441
assert_return(() => call($11, "grow1", [3]), 1);

// memory_grow.wast:442
assert_return(() => call($11, "grow1", [4]), 4);

// memory_grow.wast:443
assert_return(() => call($11, "grow1", [1]), 8);

// memory_grow.wast:444
assert_return(() => call($11, "grow2", [1]), 2);

// memory_grow.wast:445
assert_return(() => call($11, "grow2", [1]), 3);

// memory_grow.wast:450
assert_invalid("\x00\x61\x73\x6d\x01\x00\x00\x00\x01\x85\x80\x80\x80\x00\x01\x60\x00\x01\x7f\x03\x82\x80\x80\x80\x00\x01\x00\x05\x83\x80\x80\x80\x00\x01\x00\x01\x0a\x8f\x80\x80\x80\x00\x01\x89\x80\x80\x80\x00\x00\x43\x00\x00\x00\x00\x40\x00\x0b");

// memory_grow.wast:459
assert_invalid("\x00\x61\x73\x6d\x01\x00\x00\x00\x01\x85\x80\x80\x80\x00\x01\x60\x00\x01\x7f\x03\x82\x80\x80\x80\x00\x01\x00\x05\x83\x80\x80\x80\x00\x01\x00\x00\x0a\x8a\x80\x80\x80\x00\x01\x84\x80\x80\x80\x00\x00\x40\x00\x0b");

// memory_grow.wast:468
assert_invalid("\x00\x61\x73\x6d\x01\x00\x00\x00\x01\x85\x80\x80\x80\x00\x01\x60\x00\x01\x7f\x03\x82\x80\x80\x80\x00\x01\x00\x05\x83\x80\x80\x80\x00\x01\x00\x00\x0a\x8f\x80\x80\x80\x00\x01\x89\x80\x80\x80\x00\x00\x41\x00\x02\x7f\x40\x00\x0b\x0b");

// memory_grow.wast:478
assert_invalid("\x00\x61\x73\x6d\x01\x00\x00\x00\x01\x85\x80\x80\x80\x00\x01\x60\x00\x01\x7f\x03\x82\x80\x80\x80\x00\x01\x00\x05\x83\x80\x80\x80\x00\x01\x00\x00\x0a\x8f\x80\x80\x80\x00\x01\x89\x80\x80\x80\x00\x00\x41\x00\x03\x7f\x40\x00\x0b\x0b");

// memory_grow.wast:488
assert_invalid("\x00\x61\x73\x6d\x01\x00\x00\x00\x01\x85\x80\x80\x80\x00\x01\x60\x00\x01\x7f\x03\x82\x80\x80\x80\x00\x01\x00\x05\x83\x80\x80\x80\x00\x01\x00\x00\x0a\x91\x80\x80\x80\x00\x01\x8b\x80\x80\x80\x00\x00\x41\x00\x41\x00\x04\x7f\x40\x00\x0b\x0b");

// memory_grow.wast:499
assert_invalid("\x00\x61\x73\x6d\x01\x00\x00\x00\x01\x84\x80\x80\x80\x00\x01\x60\x00\x00\x03\x82\x80\x80\x80\x00\x01\x00\x05\x83\x80\x80\x80\x00\x01\x00\x01\x0a\x8c\x80\x80\x80\x00\x01\x86\x80\x80\x80\x00\x00\x41\x01\x40\x00\x0b");

// memory_grow.wast:508
assert_invalid("\x00\x61\x73\x6d\x01\x00\x00\x00\x01\x85\x80\x80\x80\x00\x01\x60\x00\x01\x7f\x03\x82\x80\x80\x80\x00\x01\x00\x05\x83\x80\x80\x80\x00\x01\x00\x01\x0a\x8f\x80\x80\x80\x00\x01\x89\x80\x80\x80\x00\x00\x43\x00\x00\x00\x00\x40\x00\x0b");

// memory_grow.wast:518
assert_invalid("\x00\x61\x73\x6d\x01\x00\x00\x00\x01\x84\x80\x80\x80\x00\x01\x60\x00\x00\x03\x82\x80\x80\x80\x00\x01\x00\x05\x83\x80\x80\x80\x00\x01\x00\x01\x0a\x8c\x80\x80\x80\x00\x01\x86\x80\x80\x80\x00\x00\x41\x00\x40\x00\x0b");

// memory_grow.wast:527
assert_invalid("\x00\x61\x73\x6d\x01\x00\x00\x00\x01\x85\x80\x80\x80\x00\x01\x60\x00\x01\x7d\x03\x82\x80\x80\x80\x00\x01\x00\x05\x83\x80\x80\x80\x00\x01\x00\x01\x0a\x8c\x80\x80\x80\x00\x01\x86\x80\x80\x80\x00\x00\x41\x00\x40\x00\x0b");
